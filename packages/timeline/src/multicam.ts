...
export function splitAngles(angles: readonly AngleCut[], offsetMs: number): { left: AngleCut[]; right: AngleCut[] } {
  ...
  const right = [
    ...
    ...angles.filter((a) => a.atMs > offsetMs).map((a) => ({ ...a, atMs: a.atMs - offsetMs })),
  ]
...
export function getAngleTransitionAt(element: MulticamElement, localMs: number): AngleTransitionWindow | null {
  ...
  for (let i = 1; i < element.angles.length; i++) {
    ...
    const half = Math.min(transition.durationMs / 2, (cut.atMs - previous.atMs) / 2, (nextAtMs - cut.atMs) / 2)
    if (half <= 0) continue
