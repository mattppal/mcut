...
`abandonedText` and skip any candidate that is a deliberate repetition, then cut
each range with a split at both ends and a ripple delete. Pass a lower
`minMatchWords` only when a short restart was missed.
