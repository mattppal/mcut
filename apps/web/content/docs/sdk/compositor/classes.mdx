...
The reference `RenderBackend` that draws directly into a 2D context. Its `renderScale` comes from the context's transform when the backend is constructed.
...
A `RenderBackend` that composites image quads as textured GPU passes with WGSL effects and uploads Canvas 2D raster content as texture layers. Its `renderScale` is the canvas width over `width`.
