...
[mcut SDK](../../../index.mdx) / [timeline/src](../index.mdx) / getMulticamAudioSource

# Function: getMulticamAudioSource()

```ts
function getMulticamAudioSource(element):
  | {
  assetId: `a-${string}`;
  key: string;
  trimStartMs: number;
...
Defined in: [packages/timeline/src/multicam.ts:82](https://github.com/mattppal/mcut/blob/main/packages/timeline/src/multicam.ts#L82)
...
```ts
assetId: `a-${string}` = assetIdSchema;
```
