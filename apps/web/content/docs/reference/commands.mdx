...
Commands live in one closed table, `commandDefinitions` in
`packages/timeline/src/commands.ts`, and `BuiltinCommand` is the union of every
entry. The table backs:

- `listCommands()` for command definitions.
- `listToolDefinitions()` for MCP-shaped tool definitions.
- `parseCommand(value)` for turning untrusted JSON into a `BuiltinCommand`.
...
| Group | Examples |
| --- | --- |
| Project and tracks | `updateProject`, `addTrack`, `removeTrack`, `renameTrack`, `setTrackFlags`, `reorderTrack`, `compactTrackGaps`, `compactTimelineGaps` |
| Assets | `addAsset`, `updateAsset`, `removeAsset` |
| Elements | `addElement`, `removeElement`, `updateElement`, `moveElement`, `trimElement`, `splitElement`, `rippleDelete`, `removeRanges` |
| Captions and derived clips | `applyCaptions`, `detachAudio`, `applyThumbnail` |
| Keyframes | `setKeyframe`, `removeKeyframe`, `moveKeyframe`, `setKeyframeEasing`, `clearKeyframes` |
| Zoom regions | `addZoomRegion`, `updateZoomRegion`, `removeZoomRegion` |
| Effects and presets | effect stack commands, animation presets, property presets |
