...
| `clean-voice` | Remove background noise from speech in a 48 kHz WAV file. `--amount` below 1 mixes the original back in. |
...
The CLI exposes the same command table that the MCP server uses.
