...
const DEFAULT_OG_DESCRIPTION = 'TypeScript packages for timelines, media, captions, CLI workflows, and MCP tools.'
...
function titleParts(title: string) {
  ...
  return [
    ...
    h('span', { key: 'agents', style: styles.highlight }, title.slice(match.index, match.index + match[0].length)),
...
function ogImage(title: string, description: string) {
  return h(
    'div',
    { style: styles.frame },
    h('div', { style: styles.top }, h('div', { style: styles.brand }, 'mcut'), h('div', { style: styles.tag }, 'video SDK + editor')),
    h('div', { style: styles.main }, h('h1', { style: styles.title }, ...titleParts(title)), h('p', { style: styles.description }, description)),
    h('div', { style: styles.bottom }, h('div', { style: styles.license }, 'Apache-2.0'), h('div', { style: styles.packages }, '@mcut/*')),
  )
}
...
function handleOgImage(request: Request) {
  ...
  const description = clampText(url.searchParams.get('description'), DEFAULT_OG_DESCRIPTION, 150)
