...
const manifest: FixtureManifest | null = existsSync(manifestFile)
...
test.beforeAll(() => {
  bundle = execFileSync(
    "bun",
    ["build", path.join(__dirname, "media-fuzz", "inpage.ts"), "--target", "browser", "--format", "iife"],
...
function probeInPage(page: Page, bytes: Buffer, type: string): Promise<InPageProbe> {
  return page.evaluate(
    ([base64, mime]) => window.__mcutMediaFuzz.probe(base64, mime),
