...
test('multicam: create from selection, style a slot shadow without crashing', async ({ page, editorUrl }) => {
  ...
  await page.locator('[data-mcut-slot-editor] > div').filter({ hasText: 'camera' }).first().click()
  ...
  await page.locator('[data-mcut-slot-editor] > div').filter({ hasText: 'screen' }).first().click()
  await page.getByRole('switch', { name: 'Shadow' }).click()
  ...
  const cameraSlot = page.locator('[data-mcut-slot-editor] > div').filter({ hasText: 'camera' }).first()
