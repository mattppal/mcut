...
export default async function ToolsPage({ searchParams }: { searchParams?: Promise<SearchParams> | SearchParams }) {
    ...
    <div className="flex flex-1 flex-col bg-background text-foreground">
      <header className="mx-auto flex w-full max-w-2xl items-center justify-between px-6 py-6">
        ...
        <nav className="flex items-center gap-5 text-sm">
          ...
          <a className="font-mono text-xs text-muted-foreground transition-colors hover:text-foreground" href={`/tools.json?profile=${profile}`}>
            tools.json
          </a>
        </nav>
      </header>
      ...
      <main className="mx-auto flex w-full max-w-2xl flex-1 flex-col px-6">
        <section className="flex flex-col gap-4 pt-16 pb-10">
          ...
          <p className="max-w-md text-sm leading-relaxed text-muted-foreground">
            ...
            <a className="underline underline-offset-2 hover:text-foreground" href={`/tools.json?profile=${profile}`}>
              /tools.json
            </a>
            .
          </p>
