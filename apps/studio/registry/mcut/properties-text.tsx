...
import { type Project, type TextBox, type TextRun, type TextStyle } from '@mcut/timeline'
...
export function TextStyleSection({
  ...
  const weightItems = weights.includes(style.fontWeight) ? weights : [...weights, style.fontWeight].sort((a, b) => a - b)
  ...
  const setFamily = (fontFamily: string) => {
    ...
    const fontWeight = next?.variableWeight
      ? Math.round(Math.min(next.variableWeight.max, Math.max(next.variableWeight.min, style.fontWeight)))
      : next
  ...
  return (
    <>
      <div className="flex items-center gap-2">
        ...
        <FontPicker value={style.fontFamily} weight={style.fontWeight} italic={italic} onSelect={setFamily} className="flex-1" />
      </div>
      {axis ? (
        <NumberField
          label="Weight"
        ...
        <FieldRow label="Weight">
          <Select value={String(style.fontWeight)} onValueChange={(w) => w && setWeight(Number(w))}>
      ...
      <NumberField label="Size" value={style.fontSize} min={4} unit="px" onCommit={(fontSize) => patchStyle({ fontSize })} />
      <NumberField
        label="Spacing"
      ...
      <ChoiceRow label="Align" value={style.align} options={['left', 'center', 'right'] as const} onCommit={(align) => patchStyle({ align })} />
      <ColorField label="Color" value={style.color} onCommit={(color) => patchStyle({ color })} />
      <ColorField label="Fill" value={style.backgroundColor ?? 'rgba(0, 0, 0, 0)'} onCommit={(backgroundColor) => patchStyle({ backgroundColor })} />
      <StrokeFields label="Outline" defaultColor="#000000" value={style.stroke} onCommit={(stroke) => patchStyle({ stroke })} />
