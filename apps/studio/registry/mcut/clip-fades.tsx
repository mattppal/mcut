...
export function FadeOverlay({ element, pxPerMs, widthPx, interactive }: { element: FadeableClip; pxPerMs: number; widthPx: number; interactive: boolean }) {
  const engine = useEditor()
  const dragRef = useRef<{ edge: 'in' | 'out'; startClientX: number; startMs: number } | null>(null)
  ...
  const onPointerMove = (event: ReactPointerEvent<HTMLSpanElement>) => {
    ...
    const nextMs = Math.round(Math.max(0, Math.min(element.durationMs, drag.startMs + deltaMs)))
    try {
      engine.dispatch({
        type: 'updateElement',
  ...
  return (
    <>
      {fadeInPx > 1 && (
        <svg className="pointer-events-none absolute inset-y-0 left-0 z-20 h-full" width={fadeInPx} preserveAspectRatio="none" viewBox="0 0 1 1">
          <path d="M 0 0 L 1 0 L 0 1 Z" fill="rgba(0, 0, 0, 0.4)" />
          <path d="M 0 1 L 1 0" stroke="rgba(255, 255, 255, 0.8)" strokeWidth={0.04} fill="none" />
        </svg>
      )}
      {fadeOutPx > 1 && (
        <svg className="pointer-events-none absolute inset-y-0 right-0 z-20 h-full" width={fadeOutPx} preserveAspectRatio="none" viewBox="0 0 1 1">
