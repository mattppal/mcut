...
describe('parseEditorPrefs', () => {
  test('stored prefs parse to the exact object', () => {
    expect(parseEditorPrefs('{"pxPerMs":0.1,"snapEnabled":false,"autoCrossfade":true,"theme":"light","previewQuality":720}')).toEqual({
      pxPerMs: 0.1,
      snapEnabled: false,
      autoCrossfade: true,
      theme: 'light',
  ...
  test('a wrong-typed pref is dropped so the provider default applies, siblings survive', () => {
    expect(parseEditorPrefs('{"pxPerMs":"wide","snapEnabled":"yes","theme":"sepia","previewQuality":-1,"autoCrossfade":true}')).toEqual({ autoCrossfade: true })
  })
