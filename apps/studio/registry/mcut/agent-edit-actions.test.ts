...
describe("agent edit action inputs", () => {
  test("a wrong-typed field is rejected with the field named", () => {
    ...
    expect(() => removeTranscriptSilence(engine, { paddingMs: "0" })).toThrow(
  ...
  test("a field outside the action's schema is rejected", () => {
    ...
    expect(() => applyOpeningClosingFades(engine, { durationMs: 500, fadeMs: 1 })).toThrow(
      '✖ Unrecognized key: "fadeMs"',
    );
  });
  ...
  test("an element id without the e- prefix is rejected before any lookup", () => {
    ...
    expect(() => applyOpeningClosingFades(engine, { elementId: "video" })).toThrow(
