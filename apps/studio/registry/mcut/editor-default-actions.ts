...
defineAction({
  ...
  enabled: ({ engine }) =>
    engine.project.tracks.some((track) =>
      track.elements.some((element) => element.type === 'video' || element.type === 'image' || element.type === 'text' || element.type === 'multicam'),
    ),
...
for (const preset of ASPECT_PRESETS) {
  defineAction({
    ...
    category: 'view',
    run: ({ engine }) => engine.dispatch({ type: 'updateProject', width: preset.width, height: preset.height }),
  })
}
...
function targetMulticam({ engine, ui }: ActionContext) {
  ...
  return findTargetMulticam(engine.project, engine.selection.elementIds, engine.playback.state.currentTimeMs)
}
