...
const registry = JSON.parse(readFileSync(path.join(studioRoot, 'registry.json'), 'utf8')) as { items: RegistryItem[] }
...
function resolveRelative(fromFile: string, spec: string): string | undefined {
  ...
  for (const candidate of [`${base}.ts`, `${base}.tsx`, base, path.join(base, 'index.tsx'), path.join(base, 'index.ts')]) {
...
describe('registry.json manifest', () => {
  ...
  test('every listed file exists on disk', () => {
    const missing = registry.items.flatMap((item) =>
      item.files.filter((file) => !existsSync(path.join(studioRoot, file.path))).map((file) => `${item.name}: ${file.path}`),
    )
  ...
  for (const item of registry.items) {
    describe(item.name, () => {
      ...
      test('lists every file its imports reach', () => {
        const unlisted = [...closure.files].filter((file) => !listed.has(file) && file !== 'lib/utils.ts' && !file.startsWith('components/ui/')).sort()
      ...
      test('declares every ui component it imports as a registryDependency', () => {
        ...
        const undeclared = [...closure.uiImports].filter((name) => !registryDeps.has(name)).sort()
        expect(undeclared).toEqual([])
      })

      test('declares every npm package it imports', () => {
        const declared = new Set((item.dependencies ?? []).map((dep) => dependencyName(dep)))
        const undeclared = [...closure.packages].filter((pkg) => !declared.has(pkg) && !FRAMEWORK_PACKAGES.has(pkg)).sort()
        expect(undeclared).toEqual([])
      })
    })
  }
