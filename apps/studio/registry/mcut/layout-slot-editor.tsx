...
function snapAxis(positions: number[], candidates: number[], threshold: number): { delta: number; guide: number | null } {
...
function axisCandidates(layout: Layout, skipIndex: number, safe: LayoutSlot['rect'], axis: 'x' | 'y'): number[] {
  const candidates = axis === 'x' ? [0, safe.x, 0.5, safe.x + safe.w, 1] : [0, safe.y, 0.5, safe.y + safe.h, 1]
...
function SlotBox({
  ...
  return (
    ...
    >
      <span className="absolute top-0.5 left-1.5 rounded-sm bg-overlay/60 px-1 text-2xs text-overlay-foreground/90 capitalize">{slot.source}</span>
      {selected &&
        !cropping &&
        HANDLES.map((handle) => (
          <span
...
export function LayoutSlotEditor() {
  ...
  const layout = useEditorState((s) => (editingLayoutId ? s.project.layouts.find((l) => l.id === editingLayoutId) : undefined))
...
function LayoutEditor({ layout }: { layout: Layout }) {
  ...
  return (
    ...
    >
      ...
      {guides.x !== null && <div className="pointer-events-none absolute inset-y-0 w-px bg-(--snap-guide)" style={{ left: `${guides.x * 100}%` }} />}
      {guides.y !== null && <div className="pointer-events-none absolute inset-x-0 h-px bg-(--snap-guide)" style={{ top: `${guides.y * 100}%` }} />}
      ...
      <div className="absolute top-2 left-1/2 flex -translate-x-1/2 items-center gap-2 rounded-lg bg-overlay/65 px-2 py-1">
        <span className="text-2xs text-overlay-foreground/90">
          {cropIndex !== null ? 'Drag inside the slot to pan its crop — Esc or double-click to finish' : `Editing “${layout.name}” — style it in the inspector`}
        </span>
        <Button
          size="xs"
          variant="secondary"
