...
import { EditorEngine, createProject, getElement, type AnimatableProperty, type ElementId, type TimelineElement } from '@mcut/timeline'
import {
...
function setKeyframes(engine: EditorEngine, elementId: ElementId, property: AnimatableProperty, times: readonly number[]) {
...
describe('timeline keyframe drag helpers', () => {
  test('Option-click deletes only when movement stays below the drag threshold', () => {
    ...
    expect(resolveKeyframeClickGesture({ moved: false, startedAltKey: true, endedAltKey: true })).toBe('delete')
    expect(resolveKeyframeClickGesture({ moved: true, startedAltKey: true, endedAltKey: true })).toBe('none')
  })

  test('plain click seeks and Option-drag disables snapping without deleting', () => {
    expect(resolveKeyframeClickGesture({ moved: false, startedAltKey: false, endedAltKey: false })).toBe('seek')
