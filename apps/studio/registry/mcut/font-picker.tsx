...
function FontRow({ option, selected, onPick }: { option: FontOption; selected: boolean; onPick: (family: string) => void }) {
  ...
  return (
    ...
    >
      ...
      {option.hasItalic && <span className="ml-auto pl-2 text-2xs text-muted-foreground italic">i</span>}
    </CommandItem>
  )
}
...
export function FontPicker({
  ...
  const groups = SOURCE_GROUPS.map(({ source, heading }) => ({
    ...
    options: library.options.filter(
      (o) => o.source === source && (category === null || o.source === 'system' || o.source === 'uploaded' || o.category === category),
    ),
        ...
        <Command>
          ...
          <CommandList className="max-h-72">
            ...
            {recents.length > 0 && (
              <CommandGroup heading="Recent">
                {recents.map((option) => (
                  <FontRow key={`recent-${option.family}`} option={option} selected={option.family === value} onPick={pick} />
                ))}
              </CommandGroup>
            )}
            {groups.map((group) => (
              <CommandGroup key={group.heading} heading={group.heading}>
                {group.options.map((option) => (
                  <FontRow key={option.family} option={option} selected={option.family === value} onPick={pick} />
                ))}
              </CommandGroup>
            ))}
          </CommandList>
          <div className="flex items-center gap-1.5 border-t border-border/60 p-1.5">
            {isSystemFontAccessSupported() && library.systemStatus !== 'ready' && (
              <Button size="xs" variant="outline" className="flex-1" disabled={library.systemStatus === 'loading'} onClick={() => void onLoadSystemFonts()}>
                {library.systemStatus === 'loading' ? 'Loading…' : 'Use system fonts'}
              </Button>
            )}
            <Button size="xs" variant="outline" className="flex-1" onClick={() => fileInputRef.current?.click()}>
              Upload font…
            </Button>
            <input
              ref={fileInputRef}
