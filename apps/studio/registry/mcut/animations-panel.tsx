...
function ZoomsSection() {
  ...
  const apply = (preset: ZoomPreset) => {
    ...
    const atMs = Math.round(Math.max(0, Math.min(playheadMs - element.startMs, element.durationMs - preset.durationMs)))
    try {
...
function ZoomSparkline({ preset }: { preset: ZoomPreset }) {
  ...
  const points = track.map((k) => `${(k.t * 44 + 2).toFixed(1)},${(14 - ((k.value - min) / span) * 12 + 1).toFixed(1)}`).join(' ')
  return (
