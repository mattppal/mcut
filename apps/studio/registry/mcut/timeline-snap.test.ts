...
const at = (...times: number[]): SnapTarget[] => times.map((timeMs) => ({ timeMs, kind: 'clip-start' as const }))
...
describe('collectSnapTargets', () => {
  test('includes zero, playhead, markers, and clip edges; excludes dragged ids', () => {
    ...
    expect(collectSnapTargets(project, 4321).map((t) => t.timeMs)).toEqual([0, 500, 900, 1300, 4321])
    expect(collectSnapTargets(project, 900, new Set([id])).map((t) => t.timeMs)).toEqual([0, 900, 900])
