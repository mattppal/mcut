...
function googleCss2Url(entry: GoogleFontEntry, text?: string): string {
  ...
  const axes = entry.italic ? `:ital,wght@${[...stops.map((w) => `0,${w}`), ...stops.map((w) => `1,${w}`)].join(';')}` : `:wght@${stops.join(';')}`
...
export function closestWeight(available: number[], wanted: number): number {
  return available.reduce((best, w) => (Math.abs(w - wanted) < Math.abs(best - wanted) ? w : best), available[0] ?? 400)
}
...
async function ensureGoogleFont(entry: GoogleFontEntry, weight: number, italic: boolean): Promise<void> {
  ...
  const min = entry.weights[0] ?? 400
  const max = entry.weights[entry.weights.length - 1] ?? 400
  const w = entry.variable ? Math.round(Math.min(max, Math.max(min, weight))) : closestWeight(entry.weights, weight)
...
function parseFontNames(buffer: ArrayBuffer): { family?: string; subfamily?: string } {
  try {
    ...
    if (tag !== SFNT_TAG.trueTypeOutlines && tag !== SFNT_TAG.cffOutlines && tag !== SFNT_TAG.appleTrueType) {
      return {}
    }
...
export async function ensureProjectFontsLoaded(project: Project): Promise<void> {
  await Promise.allSettled(collectProjectFontSpecs(project).map((spec) => ensureFontLoaded(spec.family, spec.weight, spec.italic)))
}
...
async function googleExportFaces(entry: GoogleFontEntry, specs: FontSpec[]): Promise<ExportFontFaceInit[]> {
  ...
  const wantedWeights = new Set(
    specs.map((spec) => (entry.variable ? Math.round(Math.min(max, Math.max(min, spec.weight))) : closestWeight(entry.weights, spec.weight))),
  )
