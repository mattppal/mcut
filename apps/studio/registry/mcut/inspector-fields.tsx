...
export function FieldRow({ label, title, children }: { label: React.ReactNode; title?: string; children: React.ReactNode }) {
  return (
...
const keepLabelAScrubHandleThatNeverFocusesTheInput = (event: ReactMouseEvent<HTMLLabelElement>) => event.preventDefault()
...
export function ColorField({ label, value, onCommit }: { label: string; value: string; onCommit: (value: string) => void }) {
  const engine = useEditor()
  const [draft, setDraft] = useState<string | null>(null)
  ...
  return (
    <FieldRow label={label}>
      <Popover>
        ...
        />
        <PopoverContent className="w-64 p-3" side="left" onPointerDownCapture={beginPickerGesture}>
          <ColorPicker className="flex flex-col gap-3" defaultValue={value} onChange={(rgba) => onCommit(toCssColor(rgba as [number, number, number, number]))}>
  ...
  return (
    <Collapsible defaultOpen={defaultOpen} className="group/section">
      <div className="flex items-center">
        <CollapsibleTrigger
          render={<button type="button" className={cn('flex flex-1 items-center gap-1 py-1 hover:text-foreground', panelSectionLabelClass)} />}
        >
      ...
      <CollapsibleContent className="flex flex-col gap-1.5 pt-1 pb-2">{children}</CollapsibleContent>
    </Collapsible>
  )
}
