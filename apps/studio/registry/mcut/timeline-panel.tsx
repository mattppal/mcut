...
export function TimelinePanel({ className }: TimelinePanelProps) {
  ...
  const onBackgroundPointerMove = (event: ReactPointerEvent<HTMLDivElement>) => {
    ...
    next.active = next.active || Math.hypot(next.x1 - next.x0, next.y1 - next.y0) > 5
    setMarquee(next)
...
function CurrentTime() {
  ...
  return <span className="font-mono text-2xs text-primary tabular-nums">{formatTimecode(currentTimeMs)}</span>
}
