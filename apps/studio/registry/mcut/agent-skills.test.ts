...
describe('agent skill discovery', () => {
  ...
  test('publishes the MCP-focused editing skill and its generated references', () => {
    ...
    expect(readFileSync(join(SKILLS_ROOT, 'mcut-editing/SKILL.md'), 'utf8')).toContain('**MCP server**')
    ...
    expect(existsSync(join(SKILLS_ROOT, 'mcut-editing/assets/templates/talking-head.json'))).toBe(true)
  })
})
