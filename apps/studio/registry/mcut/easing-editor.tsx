...
import { getKeyframes, type AnimatableProperty, type BuiltinCommand, type Easing, type Keyframe, type TimelineElement } from '@mcut/timeline'
...
export function EasingGraph({ element, property }: { element: TimelineElement; property: AnimatableProperty }) {
  const engine = useEditor()
  const track = getKeyframes(element, property)
  const [segment, setSegment] = useState(0)
  const svgRef = useRef<SVGSVGElement | null>(null)
  const dragRef = useRef<{ kind: 'key'; index: number; fromMs: number } | { kind: 'handle'; which: 1 | 2 } | null>(null)

  if (track.length < 2) {
    return <p className="p-2 text-xs text-muted-foreground">Add at least two keyframes to edit the curve.</p>
  }
  ...
  const onPointerMove = (event: ReactPointerEvent) => {
    ...
    if (drag.kind === 'key') {
      ...
      if (drag.index !== 0 && toMs !== drag.fromMs) {
        ...
        const clamped = Math.max(prev ? prev.timeMs + 1 : 0, Math.min(toMs, next ? next.timeMs - 1 : element.durationMs))
        dispatchOrKeepLastGoodState({
          type: 'moveKeyframe',
          elementId: element.id,
          property,
      ...
      const next: [number, number, number, number] = drag.which === 1 ? [bx, by, bx2, by2] : [bx1, by1, bx, by]
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      >
        ...
        {track.map((k, i) => (
          <rect
            ...
            className={cn('cursor-grab stroke-card', i === seg || i === seg + 1 ? 'fill-primary' : 'fill-muted-foreground')}
            strokeWidth={1}
            onPointerDown={(event) => {
              event.stopPropagation()
      ...
      <p className="text-2xs text-muted-foreground">
        Drag diamonds to retime/revalue · drag the round handles to shape the curve (above the range = overshoot) · segment {seg + 1}/{track.length - 1}
      </p>
    </div>
  )
}
...
function CurveSparkline({ bezier: [x1, y1, x2, y2] }: { bezier: [number, number, number, number] }) {
  ...
  return (
    <svg width={sw} height={sh} className="text-primary">
      <path d={`M ${sx(0)} ${sy(0)} C ${sx(x1)} ${sy(y1)} ${sx(x2)} ${sy(y2)} ${sx(1)} ${sy(1)}`} fill="none" stroke="currentColor" strokeWidth={1.25} />
    </svg>
  )
}

export function EasingEditorButton({ element, property }: { element: TimelineElement; property: AnimatableProperty }) {
  return (
    <Popover>
      <PopoverTrigger render={<Button variant="ghost" size="icon-xs" aria-label="Open curve editor" title="Curve editor (graph)" />}>
...
export function CurveEditorHost() {
  ...
  const element = useEditorState((s) => (target ? getElementLocation(s.project, target.elementId)?.element : undefined))

  return (
    <Dialog open={Boolean(target && element)} onOpenChange={(open) => !open && setCurveEditorTarget(null)}>
