...
import { dispatchSafe } from './editor-actions'
...
function projectWithForcedLayout(project: Project, multicam: MulticamElement, layoutId: string): Project {
  return {
    ...project,
    tracks: project.tracks.map((track) => ({
      ...track,
      elements: track.elements.map((element) =>
        element.id === multicam.id && element.type === 'multicam' ? { ...element, angles: [{ atMs: 0, layoutId }] } : element,
      ),
    })),
  }
}
...
function LayoutTile({
  ...
  return (
    ...
    >
      <canvas ref={draw} width={192} height={Math.round((192 * project.height) / project.width)} className="block w-full" />
      <span className="absolute top-1 left-1 rounded-sm bg-overlay/65 px-1 font-mono text-2xs text-overlay-foreground">{index + 1}</span>
      <span className="absolute right-1 bottom-1 left-1 truncate rounded-sm bg-overlay/55 px-1 py-0.5 text-2xs text-overlay-foreground/90">{layout.name}</span>
      <span
        role="button"
        tabIndex={-1}
...
export function LayoutBank({ className }: { className?: string }) {
  ...
  if (!target) {
    const selectedVideos = selection.elementIds.filter((id) => project.tracks.some((t) => t.elements.some((e) => e.id === id && e.type === 'video')))
    const count = selectedVideos.length
    return (
      <div className={cn('flex w-48 shrink-0 flex-col justify-center gap-2 p-3 text-xs text-muted-foreground', className)}>
        <p className="font-medium text-foreground">No multicam yet</p>
        <p>Fastest: in the Media bin, click your screen and camera videos — a “New multicam” panel appears where you pick which is which.</p>
        <p>From the timeline instead: select both clips (click one, shift-click the other), then:</p>
        <Button
          size="xs"
          disabled={count === 0}
          onClick={() => {
            dispatchSafe(engine, { type: 'createMulticam', sources: multicamSourcesInSelection(project, selection.elementIds) })
          }}
        >
          Create multicam
        </Button>
  ...
  return (
    <div className={cn('flex w-48 shrink-0 flex-col gap-2 overflow-y-auto scroll-mask-b p-2', className)}>
      ...
      {project.layouts.map((layout, index) => (
        <LayoutTile key={layout.id} project={project} multicam={target.element} layout={layout} index={index} active={layout.id === activeLayoutId} />
      ))}
    </div>
  )
}
