...
function read(): UserTemplate[] {
  ...
  try {
    ...
    cache = Array.isArray(raw)
      ? raw.filter((t): t is UserTemplate => t && typeof t.id === 'string' && typeof t.name === 'string' && (t.kind === 'zoom' || t.kind === 'layout'))
      : []
  } catch {
    cache = []
  }
...
export function importTemplatesJson(json: string): number {
  ...
  const fresh = incoming.filter((t): t is UserTemplate => t && typeof t.id === 'string' && !known.has(t.id))
...
export function useTemplates<P>(kind: TemplateKind): UserTemplate<P>[] {
  return useSyncExternalStore(
    ...
    () => {
      ...
      if (previous && previous.length === filtered.length && previous.every((t, i) => t === filtered[i])) {
