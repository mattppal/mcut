...
export function stepLiveGesture(engine: EditorEngine, gesture: LiveGesture, { deltaRawMs, snapping, thresholdMs }: LiveStepInput): number | null {
  ...
  switch (gesture.mode) {
    ...
    case 'roll-start':
    case 'roll-end': {
      const cutBaseMs = gesture.mode === 'roll-end' ? anchor.startMs + anchor.durationMs : anchor.startMs
    ...
    case 'ripple-start':
    case 'ripple-end': {
      const edge = gesture.mode === 'ripple-end' ? 'end' : 'start'
    ...
    case 'trim-end':
      return null
    default: {
      const exhaustive: never = gesture.mode
      return exhaustive
    }
  }
}
