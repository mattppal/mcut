...
export const repoRelativeSrc: MediaSrc = (fixture) => fixture.path

export const servedSrc =
  (origin: string): MediaSrc =>
  (fixture) =>
    `${origin}/${fixture.path}`
...
const registerAsset = (fixture: Fixture, src: string): BuiltinCommand => ({
  type: 'addAsset',
  asset: {
    id: ASSET_ID,
    kind: fixture.kind,
...
function exportResult(transcript: readonly ToolCall[]): { format?: unknown; bytes?: unknown; mimeType?: unknown } {
  const run = transcript.find(
    (call) => call.name === 'run_action' && !call.isError && call.args.actionId === EXPORT_ACTION,
  )
...
export function createTasks(srcOf: MediaSrc): E2ETask[] {
  const src = srcOf(clip)
  const fixtures = [{ id: clip.id, src }]
  const importClip = [registerAsset(clip, src), addVideoTrack, placeClip(clip)]
  const importPrompt =
    `Register the media file ${src} as asset ${ASSET_ID} (kind ${clip.kind}, ` +
  ...
  return [
    {
      ...
      fixtures,
      setup: [],
      scripted: importClip.map(call),
      score: (project) => verdictOf(placedChecks(project, src)),
    },
    {
    ...
    },
    {
      ...
      target: 'any',
      fixtures,
      setup: [...importClip, captionsFor(speech)],
      scripted: [
    ...
    },
    {
      ...
      target: 'any',
      fixtures,
      setup: [{ type: 'updateProject', width: 1920, height: 1080 }, ...importClip],
      scripted: [
...
export const TASKS: E2ETask[] = createTasks(repoRelativeSrc)
