...
const SLOW_TOOLS = new Set(['ensure_transcript', 'ensure_voice_stems', 'export_video'])
...
async function fuzz(server: McpFuzzServer, options: Options): Promise<number> {
  ...
  log(
    `${options.seeds.length} sequences of ${options.length} steps over ${tools.length} tools (${[...SLOW_TOOLS].join(', ')} left out, transcription, voice cleanup, and export can take minutes)`,
  )
