...
export async function writeStandins(outDir: string): Promise<void> {
  ...
  await run([
    ...
    'lavfi',
    '-i',
    `color=c=black:s=1280x720:r=30:d=${seconds}`,
    '-i',
    wav,
    '-f',
    'lavfi',
    '-i',
    `anoisesrc=color=pink:amplitude=0.01:r=48000:d=${seconds}`,
    '-filter_complex',
    `[0:v]${cameraFilter(lines)}[v];[1:a]adelay=${CAMERA_OFFSET_MS},highpass=f=200,lowpass=f=5000,volume=0.7,apad[s];[s][2:a]amix=inputs=2:duration=shortest[a]`,
    '-map',
    '[v]',
    '-map',
    '[a]',
  ...
  await run([
    ...
    'lavfi',
    '-i',
    `color=c=black:s=1920x1080:r=30:d=${seconds}`,
    '-vf',
    screenFilter(),
    '-t',
    seconds,
  ...
  const manifest = {
    ...
    lines: lines.map((line) => ({
      ...
      cameraStartMs: line.startMs + CAMERA_OFFSET_MS,
      ...(line.retakeOf === undefined ? {} : { retakeOf: line.retakeOf }),
    })),
