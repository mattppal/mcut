...
import { discoverPublicPackages, readPackageJson } from './lib/packages'
...
async function allPublishedPackages(): Promise<PublishedPackage[]> {
  const packages: PublishedPackage[] = []
  for (const { manifest } of await discoverPublicPackages()) {
    if (!manifest.version) fail(`${manifest.name} is missing package.json version`)
...
async function main(): Promise<void> {
  ...
  const packages = publishAll ? await allPublishedPackages() : publishedPackagesFromEnv()
  ...
  try {
    for (const pkg of packages) {
      ...
      const manifest = await readPackageJson(join(dir, 'package.json'))
