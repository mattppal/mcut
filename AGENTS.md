...
the desktop editor, has its renderer at `apps/studio` and its Electron shell at
`apps/desktop`. The docs site is at `apps/web`. The agent editing skill is at
`skills/mcut-editing`. This file is the agent contract for the whole
repository.

## Where the docs are

`apps/web/content/docs/` is the docs site. `studio/` covers installing the
desktop app and connecting an MCP client to it. `concepts/` explains one
mechanism per page. `packages/` has one page per published package. `recipes/`
gives the steps for one task. `reference/` documents the CLI, the MCP server,
the live bridge, commands, and operators. `sdk/` holds the API reference.
...
package you edited. `bun run build` rebuilds every package. `bun dev` builds
the packages, starts `next dev`, and opens the Electron app on it.
...
| Generated output. `apps/studio/public/r`, `skills/mcut-editing/references`, and the skill mirrors at `apps/web/public/.well-known/agent-skills` and `apps/studio/public/.well-known/agent-skills` are committed and regenerated by the build. `apps/web/public/r` is a build-time copy of the registry and stays out of git. | `git diff --exit-code` in CI. |
